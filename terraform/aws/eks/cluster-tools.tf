...
resource "helm_release" "calico" {
  ...
  provisioner "local-exec" {
    ...
    KUBECONFIG=${path.module}/kubeconfig kubectl patch namespace calico-system --type=merge -p '{"metadata":{"finalizers":null}}' || true 
  # Delete the Installation resource that owns the calico-system namespace  
    KUBECONFIG=${path.module}/kubeconfig kubectl patch installation default --type=merge -p '{"metadata":{"finalizers":null}}' || true
